...
describe('createUrl', () => {
  ...
  test('valid url from a path including params, query params', () => {
    const url = createURL('/test?existing-param=1', { foo: 'bar' });

    expect(url).toEqual('/test?existing-param=1&foo=bar');
  });
  test('valid url from a path including params, removing one with a query param', () => {
    const url = createURL('/test?existing-param=1', { [`existing-param`]: null });

    expect(url).toEqual('/test');
  });
  test('valid redirect url from a location including params, query params', () => {
